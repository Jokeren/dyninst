...
bool DwarfWalker::buildSrcFiles(::Dwarf * /*dbg*/, Dwarf_Die entry, StringTablePtr srcFiles) {
    ...
    for (unsigned i = 0; i < cnt; ++i) {
        srcFiles->push_back(dwarf_filesrc(df, i, NULL, NULL));
...
bool DwarfWalker::parse_int(Dwarf_Die e, bool p) {
    ...
    while(1) {
        ...
        Dwarf_Die ent = entry();
        //auto offset_c = dwarf_dieoffset(&current_cu_die);
        auto offset_e = dwarf_dieoffset(&ent); 
        bool is_info = !dwarf_hasattr_integrate(&ent, DW_TAG_type_unit);
        Dwarf_Die * cu_die_p = 0; 
        //auto func = is_info ? dwarf_offdie : dwarf_offdie_types; 
        //cu_die_p = func(dbg(), offset_e + offset_c, &siblingDwarf);
        if(is_info){
            cu_die_p = dwarf_offdie(dbg(), offset_e, &siblingDwarf);
        }else{
            cu_die_p = dwarf_offdie_types(dbg(), offset_e, &siblingDwarf);
        }
...
bool DwarfWalker::parseCallsite()
{
    ...
    DWARF_FAIL_RET(has_file);
    if (!has_file)
        return true;
    has_line = dwarf_hasattr(&e, DW_AT_call_line);
    DWARF_FAIL_RET(has_line );
    if (!has_line)
        return true;
...
pair<AddressRange, bool> DwarfWalker::parseHighPCLowPC(::Dwarf * dbg, Dwarf_Die entry)
{
    ...
    DWARF_FAIL_RET_VAL(form, result);
...
bool DwarfWalker::parseArray() {
   ...
   */
   ...
   Dwarf_Die e = entry();
   DWARF_FAIL_RET(dwarf_child(&e, &firstRange));
...
bool DwarfWalker::hasDeclaration(bool &isDecl) {
    bool tmp;
    Dwarf_Die e = entry();
    tmp = dwarf_hasattr(&e, DW_AT_declaration);
    DWARF_FAIL_RET(tmp);
    isDecl = tmp;
    return true;
}
...
bool DwarfWalker::handleAbstractOrigin(bool &isAbstract) {
    ...
    DWARF_FAIL_RET(isAbstractOrigin);
...
bool DwarfWalker::handleSpecification(bool &hasSpec) {
    ...
    DWARF_FAIL_RET(hasSpecification);
...
bool DwarfWalker::findDieName(::Dwarf * /*dbg*/, Dwarf_Die die, std::string &name)
{
    ...
    if (cname == 0) {
        cerr << "Error message:" << dwarf_errmsg( dwarf_errno() ) << endl;
...
bool DwarfWalker::findType(Type *&type, bool defaultToVoid) {
    ...
    Dwarf_Attribute typeAttribute;
    Dwarf_Die e = specEntry();
    auto status = dwarf_attr( &e, DW_AT_type, &typeAttribute);
    DWARF_CHECK_RET(status == 0);
        ...
        std::vector<VariableLocation> &locs) 
{
    ...
    bool hasAttr = false;
    Dwarf_Die e = entry();
    hasAttr = dwarf_hasattr(&e, attr);
    DWARF_FAIL_RET(hasAttr);
        ...
        Type * elementType)
{
    ...
    DWARF_FAIL_RET_VAL(dieOffset, NULL);
    ...
    auto cu_die_off = dwarf_dieoffset(&range); 
    bool is_info = !dwarf_hasattr_integrate(&range, DW_TAG_type_unit);
    Dwarf_Die * cu_die_p = 0; 
    if(is_info){
        cu_die_p = dwarf_offdie(dbg(), cu_die_off, &nextSibling);
    }else{
        cu_die_p = dwarf_offdie_types(dbg(), cu_die_off, &nextSibling);
    }
        ...
        Address * initialStackValue)
{
    ...
    for (unsigned locIndex = 0 ; locIndex < listLength; locIndex++) {
        ...
        /*FIXME if (location->ld_lopc == max_addr) {
